...
def set_background(image_file):
    ...
    st.markdown(
        ...
        div.stButton > button:first-child {{
            background-color: rgba(0, 0, 50, 0.85) !important; /* Mismo color del fondo del chat */
            color: white !important; /* Texto blanco */
            font-weight: bold;
            border-radius: 8px;
            padding: 0.5rem 1rem;
            border: 1px solid #ccc; /* Puedes ajustar el borde si lo deseas */
        }}
        </style>
        """,
...
@st.cache_resource
def load_documents():
    loader = PyPDFLoader("Guión_HMI.pdf")
    docs = loader.load()
